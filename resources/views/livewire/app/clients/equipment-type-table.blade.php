<div>
    ...
    <x-ui.page-header
        ...
        description="Administra los tipos de equipo disponibles para clasificar el activo instalado en tus clientes."
        :count="$equipmentTypes->total()"
        countLabel="tipos de equipo"
    >
        <x-slot:actions>
                    ...
                    <div class="flex items-center justify-end gap-1">
                        @if($item->trashed())
                            <x-ui.button
                                ...
                                aria-label="Restaurar tipo de equipo" title="Restaurar tipo de equipo" />
                        ...
                        @else
                            <x-ui.action-menu>
                                <x-ui.action-menu.item
                                    x-data @click="$dispatch('open-modal', 'edit-tipoEquipo-{{ $item->id }}')"
            ...
            <tr>
                <td colspan="{{ count($visibleColumns) + 1 }}" class="px-6 py-12">
                    <x-ui.empty-state variant="simple" title="Sin resultados"
                        description="No hay tipos de equipo registrados con los filtros aplicados." />
                </td>
            </tr>
        @endforelse
    ...
    @include('clients.equipmentTypes.partials.modals', ['equipmentsTypes' => $equipmentTypes])
</div>
