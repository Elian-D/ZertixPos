<x-app-layout title="Crear Nuevo Rol">

    <div class="py-6 sm:py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <h2 class="text-xl font-medium text-gray-700 mb-6 px-1">{{ __('Crear Nuevo Rol') }}</h2>

            <form action="{{ route('config.roles.store') }}" method="POST" x-data="{}">
                @csrf

                @include('roles.partials.form')

                <div class="flex justify-end space-x-4 pt-6 mt-6">
                    <x-ui.button href="{{ route('config.roles.index') }}" appearance="ghost" variant="secondary">
                        Cancelar
                    </x-ui.button>

                    <x-ui.button type="submit" variant="primary" iconLeft="heroicon-s-check">
                        Guardar Rol
                    </x-ui.button>
                </div>
            </form>
        </div>
    </div>
</x-app-layout>
