...
class ClientService
{
    ...
    public function createQuickClient(array $data): Client
    {
        ...
        if (empty($clientData['tax_identifier_type']) && ! empty($clientData['tax_id'])) {
            $clientData['tax_identifier_type'] = $this->resolveTaxType($clientData['tax_id']);
        }
    ...
    /**
     * Lógica básica para RD: RNC (9 dígitos) o Cédula (11 dígitos)
     */
    private function resolveTaxType(?string $taxId): ?string
    {
        if (! $taxId) {
            return null;
        }
        ...
        return match ($length) {
            9 => TaxIdentifierType::RNC->value,
            11 => TaxIdentifierType::CEDULA->value,
            default => null
        };
    }
